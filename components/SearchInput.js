...
export default function SearchInput({searchData}) {
  ...
  const handleChange = value => {
    if (!value) {
      console.log('handleChange clear')
      searchData('')
      activeStyle()
    }
    setSearchText(value)
  ...
  // 
  // onFocus={changeFocusInputStyle} onBlur={changeBlurInputStyle}
  // 

  return <>
    <div style={searchInputStyle} className={searchInputStyle.border !== "none" ? styles.searchInputChange : styles.searchInput}>
      <div onClick={showResult} onMouseOver={changeStyleHover} onMouseOut={removeHover} style={searchInputImgDivStyle} className={searchInputImgDivStyle.background !== "none" ? styles.searchInputImgDivChange : ""}>
      ...
      <input onFocus={changeFocusInputStyle} onBlur={changeBlurInputStyle} value={searchText} onChange={e => handleChange(e.target.value)} type="text" placeholder="Имя питомца"/>
    </div>
  </>
